...
function Dashboard() {
  ...
  const hasMoreFiles = filteredFiles.length < files.length || files.length % perPage === 0;
    ...
    try {
      ...
      if (snapshot.exists()) {
        ...
        const fileArray = Object.values(data).map((file) => ({
          ...file,
          id: file.public_id, // Ensure each file has a unique ID
        }));
  
        const startIndex = (page - 1) * perPage;
        const endIndex = startIndex + perPage;
        const paginatedFiles = fileArray.slice(startIndex, endIndex);
  
        // If it's the first page, reset the state, otherwise append
        if (page === 1) {
          setFiles(paginatedFiles);
          setFilteredFiles(paginatedFiles);
        } else {
          setFiles((prevFiles) => [...prevFiles, ...paginatedFiles]);
          setFilteredFiles((prevFiles) => [...prevFiles, ...paginatedFiles]);
        }
      } else {
        setFiles([]);
        setFilteredFiles([]);
  ...
  useEffect(() => {
    fetchFiles(page); // Fetch files for the current page
  }, [page]); // Run this effect when the page changes
